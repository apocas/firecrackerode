...
Firecracker.prototype.spawn = function (binPath) {
  ...
  this.child.stderr.on('data', (data) => {
    console.error(`Firecracker stderr:\n${data}`);
  });

  this.child.on('exit', function (code, signal) {
    fs.unlink(self.options.socketPath, () => { });
    console.log('Firecracker exited with ' + `code ${code} and signal ${signal}`);
    self.child = undefined;
  });

  this.child.on('error', function (err) {
    fs.unlink(self.options.socketPath, () => { });
    console.log(`Firecracker error:\n${err}`);
  });
};
